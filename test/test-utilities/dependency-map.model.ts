...
export class DependencySubstituteMap {
    private _unsafeMap = new Map<Type<unknown>, SubstituteOf<object>>();
    ...
    public get<TDep>(dependencyType: Type<TDep>): SubstituteOf<TDep> {
        ...
        if (substitute === undefined)
            throw new Error(
                `DependencySubstituteMap: Substitute for ${dependencyType.name} is not registered.`
            );

        return substitute as SubstituteOf<TDep>;
    }
