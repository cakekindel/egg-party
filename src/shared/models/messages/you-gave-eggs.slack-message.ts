...
export class SlackMessageYouGaveEggs extends SlackBlockMessage {
    constructor(
        userIds: string[],
        numberOfEggsGivenEach: number,
        numberOfEggsLeft: number
    ) {
        const mentionsList = SlackMessageYouGaveEggs.getReadableMentionsList(
            userIds
        );
        const numberOfEggsString =
            numberOfEggsGivenEach === 1
                ? 'an egg'
                : `${numberOfEggsGivenEach} eggs`;

        super([
            SlackMessageSectionBlock.fromText(
                `You gave ${numberOfEggsString} ${
                    userIds.length > 1 ? 'each to' : 'to'
                } ${mentionsList}!`
            ),
            SlackMessageContextBlock.fromText(
                `_(You have ${numberOfEggsLeft} eggs left to give today)_`
            ),
        ]);
    }
