...
import { GuideBookPageGroup, GuideBookPageId, IGuideBookPage } from '../../../shared/models/guide-book';
...
export class SlackGuideBookService
{
    ...
    public build(userId: string, botUserId: string, pageId: GuideBookPageId = GuideBookPageId.Welcome): BlockMessage
    {
        ...
        const message = new BlockMessage(
            [
                new Divider(),
                new Section(new Text(TextType.Markdown, `*:book:   Guide Book*`), undefined, jumpToPageSelect),
                new Divider(),
    ...
    private buildGuideBookContent(userId: string, botUserId: string, pageId: GuideBookPageId): ILayoutBlock[]
    {
        switch (pageId)
        {
    ...
    private buildWelcomePage(): ILayoutBlock[]
    {
        return [
            ...
            new Context([new Text(TextType.Markdown, `_Created by <https://github.com/cakekindel|Orion Kindel>_`)]),
            new Section(new Text(TextType.Markdown, ` `)),
            new Section(new Text(TextType.Markdown, `Egg Party is a new way to give thanks to your team and have fun doing it! Give your teammates eggs to show appreciation, and raise the eggs that hatch!`)),
            new Section(new Text(TextType.Markdown, `Use the "Jump to Page" menu above to learn more, or play it by ear and learn as you go!`)),
            new Context([new Text(TextType.Markdown, `> _"A great friend is he who shares an egg with a teammate that did a good job resolving the outage from their 5PM Friday deploy."_ - Plato, 413 A.D. *ᵖʳᵒᵇᵃᵇˡʸ*`)]),
        ];
    }

    private buildGivingEggsPage(userId: string, botUserId: string): ILayoutBlock[]
    {
        return [
            new Section(new Text(TextType.Markdown, `*How to Give Eggs*`)),
            new Section(new Text(TextType.Markdown, ` `)),
            new Section(new Text(TextType.Markdown, `First, invite me to the channel where the magic happens.`)),
            new Context([new Text(TextType.Markdown, `_*Pro Tip*: You can invite me to a channel super quickly by sending "/invite <@${botUserId}>"_`)]),
            new Section(new Text(TextType.Markdown, `Then, to give an egg to a teammate, send a message mentioning them with some \`:egg:\` emojis! Starting out, you can give *5 eggs a day*.`)),
            new Context([new Text(TextType.Markdown, `_*Example*: Great work this week <@${userId}>! :egg::egg::egg:_`)]),
            new Section(new Text(TextType.Markdown, `You can also give eggs by adding an :egg: reaction! This is good if you want to keep clutter down, or just really appreciate a message someone sends.`)),
        ];
    }
    ...
    private buildChickensPage(): ILayoutBlock[]
    {
        return [
            ...
            new Section(
                ...
                undefined,
                new Button(SlackInteractionId.ManageChickens, new Text(TextType.Plaintext, `Sure, I'd love to name them!`))
            ),
            new Context([new Text(TextType.Markdown, `_Nobody will see these names other than you, so feel free to get creative!_`)]),
        ];
    }
    ...
    {
        return [
            new Section(new Text(TextType.Markdown, `*Getting & Raising Chicks*`)),
            new Section(new Text(TextType.Markdown, ` `)),
            new Section(new Text(TextType.Markdown, `As mentioned in *Chickens & Daily Eggs*, when a Daily Egg is laid, there's a small chance it will hatch into a chick.`)),
            new Section(new Text(TextType.Markdown, `Daily Eggs don't get replaced unless they're given away, so be sure to give all your Daily Eggs away to maximize your chances of hatching!`)),
            new Section(new Text(TextType.Markdown, `When a Chick does hatch, you can feed it once a day. You have to feed it 3 times for it to grow up into a Chicken.`)),
            new Section(new Text(TextType.Markdown, `The food you give Chicks is very important, as it will determine what Breed it will be as a Chicken! For example, corn :corn: will yield a Delaware Hen, the basic Chicken Breed.`)),
        ];
    }
    ...
    private guideBookPageCommands(): ILayoutBlock[]
    {
        return [
            ...
            new Section(new Text(TextType.Markdown, ` `)),
            new Section(new Text(TextType.Markdown, `There are a few commands to help you on your yolky journey. To use them, simply send them here in this Direct Message.`)),
            new Context([new Text(TextType.Markdown, '`help`'), new Text(TextType.Markdown, `_Resend the welcome message_`)]),
            new Context([new Text(TextType.Markdown, '`chickens`'), new Text(TextType.Markdown, `_View & rename your chickens_`)]),
            new Context([new Text(TextType.Markdown, '`leaderboard`'), new Text(TextType.Markdown, `_View your team's Egg Party stats, and see how you stack up!_`)]),
            new Context([new Text(TextType.Markdown, '`profile`'), new Text(TextType.Markdown, `_View your own Egg Party stats_`)]),
        ];
    }
}
