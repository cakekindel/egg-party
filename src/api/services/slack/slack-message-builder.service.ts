...
export class SlackMessageBuilderService {
    ...
    public testGiveEggsResponse(
        ...
        return new BlockMessage(
            [],
            `You gave ${mentions.join(', ')} each ${eggCount} egg(s)!`
        );
    }
    ...
    public manageChickens(chickens: Chicken[]): BlockMessage {
        ...
        for (const chicken of chickens) {
            ...
            const chickenSection = new Section(
                new Text(TextType.Markdown, `:chicken: *${chicken.name}*`),
                undefined,
                renameButton
            );
    ...
    public renameChicken(chicken: Chicken): BlockMessage {
        return new BlockMessage([
            new Section(
                new Text(
                    TextType.Markdown,
                    `No problem! To rename :chicken: *${chicken.name}*, just reply with the new name.`
                )
            ),
        ]);
    }
    ...
    public chickenRenamed(oldName: string, newName: string): BlockMessage {
        const notes = [
            ...
            `Try saying it 5 times fast: "${newName} ${newName} ${newName} ${this.garbleText(
                newName
            )} ${this.garbleText(newName)}" - man I suck at tongue twisters`,
        ];
        ...
        return new BlockMessage([
            new Section(
                new Text(
                    TextType.Markdown,
                    `:chicken: *${oldName}* is now :chicken: *${newName}*! ${this.getRandomEntry(
                        notes
                    )}`
                )
            ),
