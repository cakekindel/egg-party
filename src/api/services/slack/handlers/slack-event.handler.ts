...
export class SlackEventHandler {
    ...
    public async handleEvent(event: ISlackEvent): Promise<void> {
        if (this.eventIsMessage(event)) {
            ...
            return this.messageHandler.handleMessage(event.event);
        } else if (this.eventIsReaction(event)) {
            return this.reactionHandler.handleReaction(event);
        }
    }

    private eventIsMessage(
        event: ISlackEvent
    ): event is ISlackEventWrapper<ISlackEventMessagePosted> {
        return (
            event.type === SlackEventType.EventWrapper &&
            (event as ISlackEventWrapper).event.type ===
                SlackEventType.MessagePosted
        );
    }

    private eventIsReaction(
        event: ISlackEvent
